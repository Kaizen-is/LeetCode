...
with cte as (
select id, visit_date as today, 
        lag(visit_date) over (order by id) as next_d, people
from Stadium

),
new as (
    select id, id - ROW_NUMBER() OVER (ORDER BY id) AS grp, today as visit_date, people
from cte
where people >= 100 
-- and today > next_d 
order by id
